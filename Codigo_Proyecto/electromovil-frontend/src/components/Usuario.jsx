...
const Usuario = () => {
  ...
  const handleLogout = async () => {
    try {
      // 1. Intenta cerrar sesión en el backend
      await api.logout(); // Usamos el método logout de tu api.js
      
      // 2. Eliminar el token y cualquier otro dato de usuario
      localStorage.removeItem('auth_token');
      // Agrega aquí cualquier otro dato que necesites limpiar
      
      // 3. Redirigir al login con un estado de logout exitoso
      navigate('/LoginRegister', { state: { logoutSuccess: true } });
      
    } catch (error) {
      console.error('Error al cerrar sesión:', error);
      // Si falla la petición, igual limpiamos el token localmente
      localStorage.removeItem('auth_token');
      navigate('/LoginRegister', { state: { logoutSuccess: true } });
    }
  };
      ...
      <header className="header">
        <nav className="navbar">
          ...
          <div className="nav-buttons">
            <button onClick={() => window.location.href = '../UserProfile'} className="btn-profile">
